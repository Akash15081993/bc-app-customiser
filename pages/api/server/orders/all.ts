  ...
  try {
    ...
    try {
      ...
      const [items, totalResult]: any = await Promise.all([
        mysqlQuery(
          "SELECT `id`,`orderId`,`order_items_total`, order_total_inc_tax, order_total_ex_tax FROM bcOrders WHERE storeHash = ? GROUP BY orderId ORDER BY id DESC LIMIT ? OFFSET ?",
