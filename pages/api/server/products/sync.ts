...
export default async function list(req: NextApiRequest, res: NextApiResponse) {
    try {
        ...
        if(req.method == "POST"){
            ...
            const product = await bigcommerce.get(`/catalog/products/${productId}?include=images`) 

            if(!product?.data) return res.status(404).json({ status: false, message: 'Product not found.' });
            ...
            //Update Product
            await mysqlQuery('UPDATE products SET productName = ?, productSku = ?, productImage = ? WHERE storeHash = ? AND id = ? AND productId = ?', [productName, productSku, productImage, storeHash, id, productId]);  
